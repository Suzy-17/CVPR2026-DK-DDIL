...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment, args):
        ...
        if "increment_per_session" in args:
            # 使用配置中的增量序列
            self._increments = args["increment_per_session"]
            # 验证总类别数匹配
        ...
        # 新增任务数校验
        if len(self._increments) != self.nb_tasks:
            raise ValueError(f"配置任务数{len(self._increments)}与数据任务数{self.nb_tasks}不一致")
